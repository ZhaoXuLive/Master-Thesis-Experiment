 function [out, FxL, FxR, slipL, slipR, FyL, FyR, FzL, FzR] = Maaav_magic(x, avp, u, Trq, optimal_params_latL, optimal_params_latR)
...
FyL = magic_formula(optimal_params_latL, [slipL FzL']);
FyR = magic_formula(optimal_params_latR, [slipR FzR']);
% if sum(slipL) < 0.01
%     FyL = zeros(8, 1);
% end
% if sum(slipR) < 0.01
...
Tau_tire = L_long1 * (Delta_c * FxL' - Delta_s * (-FyL')) ...
         + L_long2 * (Delta_c * FxR' - Delta_s * (-FyR')) ...
         + L_lat1 * (Delta_s * FxL' + Delta_c * (-FyL')) ...
         + L_lat2 * (Delta_s * FxR' + Delta_c * (-FyR'));
