function out = Maaav_improved_ESINDy_discrep(Y, U, T, Trq, Xi_iter, avp, sep, ...
...
for iter = 1:length(sep(:, 1))
    ...
    for i = sep(iter, 1) + 1:sep(iter,2)
        ...
        vars = [sin(out(i - 1, 3)) sin(out(i - 1, 4)) sin(out(i - 1, 5)) ...
                ...
                out(i - 1, 6:10) U(i - 1, :) Trq(i - 1, 1:2) Trq(i - 1, 7:8)];
        ...
        for k = 1:m
            yPool(:, k) = functions{k}(vars_norm(1), vars_norm(2), vars_norm(3), ...
                      ...
                      vars_norm(17), vars_norm(18), vars_norm(19), vars_norm(20));
        end
        disc = yPool * Xi_iter;
