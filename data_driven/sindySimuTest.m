...
if functionMode == 1
    ...
    Theta = basic_function_im(Y, U, Trq, polyorder);
end
if functionMode == 2
    % sine 基础改进形式：引入sin，但无脑
    Theta = sine_function_im(Y, U, Trq, polyorder);
end
if functionMode == 3
    % mechanic 机理引导的选择
    Theta = selected_function_im(Y, U, Trq, polyorder);
end
if functionMode == 4
    % standard
    Theta = standard_function_im(vars_normalized, polyorder);
end
...
if regressionMode == 3
    [Xi, Xintercept] = regression_ridge_new(Theta, Ef);
end
if regressionMode == 4
    [Xi, Xintercept] = regression_elastic_new(Theta, Ef);
end
if regressionMode == 5
    % STRidge 参数
    lambda = 3;     % 正则化参数
    tau = 0.001;    % 稀疏化阈值
    max_iter = 10;  % 最大迭代次数
    Xi = regression_stridge_new(Theta, Ef(:, 6:10), lambda, tau, max_iter);
end

% if functionMode == 1
%     XM = Maaav_SINDy_discrep_basic_im(Y, U, T, Trq, Xi, Xintercept, avp, polyorder, sep);
% end
% if functionMode == 2
%     XM = Maaav_SINDy_discrep_sine_im(Y, U, T, Trq, Xi, Xintercept, avp, polyorder, sep);
% end
% if functionMode == 3
%     XM = Maaav_SINDy_discrep_selected_im(Y, U, T, Trq, Xi, Xintercept, avp, polyorder, sep);
% end
% if functionMode == 4
%     XM = Maaav_SINDy_discrep_standard_im(Y, U, T, Trq, Xi, Xintercept, avp, polyorder, sep, vars_mean, vars_std);
% end
% 
% figure,
...
if functionMode == 1
    XM_t = Maaav_SINDy_discrep_basic_im(TestY, TestU, TestT, TestTrq, Xi, Xintercept, avp, polyorder, sepT);
end
if functionMode == 2
    XM_t = Maaav_SINDy_discrep_sine_im(TestY, TestU, TestT, TestTrq, Xi, Xintercept, avp, polyorder, sepT);
end
if functionMode == 3
    XM_t = Maaav_SINDy_discrep_selected_im(TestY, TestU, TestT, TestTrq, Xi, Xintercept, avp, polyorder, sepT);
end
if functionMode == 4
    XM_t = Maaav_SINDy_discrep_standard_im(TestY, TestU, TestT, TestTrq, Xi, Xintercept, avp, polyorder, sepT, vars_mean, vars_std);
end
