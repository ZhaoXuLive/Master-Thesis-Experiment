function out = Maaav_SINDy_discrep_selected_im(Y, U, T, Trq, Xi, Xintercept, avp, polyorder, sep) %, Xi_sparse_indices)
...
for iter = 1:length(sep(:, 1))
    ...
    for i = sep(iter, 1) + 1:sep(iter,2)
        ...
        yPool = selected_function_im(out(i - 1, 3:10), U(i - 1, :), Trq(i - 1, :), polyorder);
