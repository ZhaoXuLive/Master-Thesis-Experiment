function out = Maaav_SINDy_discrep_standard_im(Y, U, T, Trq, Xi, Xintercept, avp, polyorder, sep, vars_mean, vars_std) %, Xi_sparse_indices)
...
for iter = 1:length(sep(:, 1))
    ...
    for i = sep(iter, 1) + 1:sep(iter,2)
        ...
        vars = [out(i - 1, 3:10) U(i - 1, :) Trq(i - 1, :)];
        vars_norm = (vars - vars_mean) ./ vars_std;

        yPool = standard_function_im(vars_norm, polyorder);
